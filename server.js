...
let SFL = JSON.parse(fs.readFileSync('./sfa.json', 'utf8')); // الملف الي بتحط به الحسابات الفل داتا
let NFL = JSON.parse(fs.readFileSync('./nfa.json', 'utf8')); // الملف الي بتحط به الحسابات العاديه
let SFLP = 6000; /*سعر الحساب الواحد الفل داتا*/
let NFLP = 500; /*سعر الحساب الواحد العادي*/
let U = '535864833380450374' //مين بيتحوله الكريديت// ahmeD_Hossam
client.on('message', async message => { // ahmeD_Hossam
    let bOn = await db.fetch(`bOn_${message.guild.id}`)
    if (message == prefix + 'hdajksdhksahdahsdkahsdahskdhajshdjahsdksadsadahdhadhhdshadhsahdadhkdahhhhhhhhhhhsdadas') {
        let ahmed = 0;
        let hossam = 0; // ahmeD_Hossam
